...
public class TransaksiController {
    ...
    public void insertTiket(Tiket t) {
        try {
            if (t.getId_tiket() > 0) {
                ...
                this.koneksi.ManipulasiData("UPDATE RYVANAS_07014.PEMBELI SET NAMA_PEMBELI='"+t.getPembeli().getNama_pembeli()+"', TANGGAL_LAHIR='"+t.getPembeli().getTanggal_lahir()+"', EMAIL='"+t.getPembeli().getEmail()+"', NO_TELP="+t.getPembeli().getNo_telp()+" WHERE ID_PEMBELI="+t.getId_pembeli());
                
                this.koneksi.ManipulasiData("UPDATE RYVANAS_07014.TIKET SET ID_KERETA="+t.getId_kereta()+", ID_RUTE="+t.getId_rute()+", ID_PEMBELI="+t.getId_pembeli()+", HARGA_TIKET="+t.getHarga_tiket()+", TANGGAL_KEBERANGKATAN='"+t.getTanggal_keberangkatan()+"' WHERE ID_TIKET="+t.getId_tiket());
                
            } else {
                System.out.println("INSERT");
                
                this.koneksi.ManipulasiData("INSERT INTO RYVANAS_07014.PEMBELI VALUES(ID_PEMBELI.NEXTVAL, '"+t.getPembeli().getNama_pembeli()+"', '"+t.getPembeli().getTanggal_lahir()+"', '"+t.getPembeli().getEmail()+"', "+t.getPembeli().getNo_telp()+")");
                ...
                this.koneksi.ManipulasiData("INSERT INTO RYVANAS_07014.TIKET VALUES(ID_TIKET.NEXTVAL, "+t.getId_kereta()+", "+t.getId_rute()+", "+idPembeli+", "+t.getHarga_tiket()+", '"+t.getTanggal_keberangkatan()+"')");
            }
        } catch (Exception e) {
    ...
    public void insertRute(Rute r) {
        try {
            if (r.getId_rute() > 0) {
                System.out.println("UPDATE");
                this.koneksi.ManipulasiData("UPDATE RYVANAS_07014.RUTE SET TUJUAN='" + r.getTujuan() + "', STASIUN_PERSINGGAHAN='" + r.getStasiun_persinggahan() + "' WHERE ID_RUTE=" + r.getId_rute());
            } else {
                System.out.println("INSERT");
                this.koneksi.ManipulasiData("INSERT INTO RYVANAS_07014.RUTE VALUES(ID_RUTE.NEXTVAL, '" + r.getTujuan() + "', '" + r.getStasiun_persinggahan() + "')");
            }
        } catch (Exception e) {
            System.out.println(e);
        }
    ...
    public void insertKereta(Kereta k) {
        try {
            if (k.getId_kereta() > 0) {
                System.out.println("UPDATE");
                this.koneksi.ManipulasiData("UPDATE RYVANAS_07014.KERETA SET NAMA_KERETA='" + k.getNama_kereta() + "', JADWAL='" + k.getJadwal() + "', NOMOR_KURSI=" + k.getNomor_kursi() + " WHERE ID_KERETA=" + k.getId_kereta());
            } else {
                System.out.println("INSERT");
                this.koneksi.ManipulasiData("INSERT INTO RYVANAS_07014.KERETA VALUES(ID_KERETA.NEXTVAL, '" + k.getNama_kereta() + "', '" + k.getJadwal() + "', " + k.getNomor_kursi() + ")");
            }
        } catch (Exception e) {
            System.out.println(e);
        }
